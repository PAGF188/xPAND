...
__all__ = ["fast_rcnn_inference", "BoxVerificationFastRCNNOutputLayers"]
...
class BoxVerificationFastRCNNOutputLayers(nn.Module):
    """
    One linear layers for predicting the IoU:
    ...
    def __init__(
        ...
        *,
        ...
        # Prediction layer for IoU estimation
        self.iou_score = nn.Linear(input_size, 1)

        nn.init.normal_(self.iou_score.weight, std=0.01)
        for l in [self.iou_score]:
            nn.init.constant_(l.bias, 0)

        if isinstance(loss_weight, float):
            loss_weight = {"loss_iou": loss_weight}
    ...
    def losses(self, predictions, proposals):
        ...
        # parse iou outputs
        gt_ious = (cat([p.ious for p in proposals], dim=0))
    
        loss_iou = F.binary_cross_entropy_with_logits(predictions[:,0].float(), gt_ious, reduction="mean")
