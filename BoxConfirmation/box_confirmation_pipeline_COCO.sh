...
python3 pipeline_scripts/1_auxiliary_add_dumm_iou_BV.py --gt /datasets/cocosplit/datasplit/trainvalno5k.json \
    --dets ${pseudo_path}/${pseudo_initial_name}_filtrado_base2_nms_filt_score0.2_LC0.5.json

TEST_NAME=${method}_coco_pseudos_${shot}shot_seed${seed}_iter${iter}  # MAKE SURE YOU HAVE REGISTERED THE CC OUTPUT DATASETS IN builtin.py !!!
OUTPUT_DIR=${pseudo_path}/BC_output/
WEIGHTS=/models/BC_COCO/${shot}shot_seed${seed}/model_final.pth
python3 /detectron2/tools/lazyconfig_train_net.py --eval-only --num-gpus $GPUS --config-file configs/box_ref_test.py \
...
python3 pipeline_scripts/2_iou_filtering_BV.py --pseudos ${pseudo_path}/${pseudo_initial_name}_filtrado_base2_nms_filt_score0.2_LC0.5.json \
    --box_ver_output ${pseudo_path}/BC_output/${method}_coco_pseudos_${shot}shot_seed${seed}_iter${iter}_ubbr.json \
    --th 0.8
